...
def financial_summary(symbol: str):
    ...
    prompt = """
    Berdasarkan data keuangan berikut dari {data}:
    
    Berikan ringkasan singkat tentang kinerja keuangan perusahaan ini dalam 3-5 poin utama.
    """
...
def revenue_trend(symbol: str, financials: pd.DataFrame):
    ...
    prompt = f"""
    Anda adalah seorang programmer Python yang sangat ahli dalam visualisasi data.
...
def risk_analysis(financials: pd.DataFrame):
    ...
    prompt = """ Anda adalah seorang analis risiko keuangan yang skeptis.
